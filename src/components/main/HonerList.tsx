...
function getHonorList() {
  ...
  const honorList = testData.sort((a, b) => b.id - a.id).map((m) => {
...
function renderRow(props: ListChildComponentProps) {
  ...
  return (
    <ListItem style={style} key={index} component="div" disablePadding>
      <ListItemButton>
        <ListItemText primary={`${getHonorList()[index].id}" ${getHonorList()[index].team}`} />
      </ListItemButton>
    </ListItem>
  );
}
...
export default function VirtualizedList() {
  return (
    <>
    <Box
      sx={{ width: '100%', height: 400, maxWidth: 400, bgcolor: '#282934' }}
    >
      <FixedSizeList
        height={400}
        width={360}
